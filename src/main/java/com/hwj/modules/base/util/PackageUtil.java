...
public class PackageUtil {
	...
	protected static List<String> processPackages(List<String> packages, String basePackage, boolean mapperAllClasses) {
		...
		try {
			Resource[] resources = resolver.getResources(ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX + "/"
					+ basePackage + (mapperAllClasses ? ALL_CLASS_SURFFIX : ""));
			for (Resource resource : resources) {
				...
				if (packages.contains(packageStr.replace("/", ".")) == false)
					packages.add(packageStr.replace("/", "."));
			}
		} catch (IOException e) {
			//
		}
	...
	public static void main(String[] args) {
		List<String> list = getPackages("com.zlcao.**.model", true);
